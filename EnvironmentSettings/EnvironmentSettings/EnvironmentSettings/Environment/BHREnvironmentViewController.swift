...
enum EnvironmentSection: Int {
  case endPoint, oAuth, featureOverride, featureToggles
}
...
extension BHREnvironmentViewController: UITableViewDataSource, UITableViewDelegate {
  ...
  func tableView(_: UITableView, titleForHeaderInSection section: Int) -> String? {
    switch section {
    case EnvironmentSection.endPoint.rawValue:
      return "End Points"
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    case EnvironmentSection.endPoint.rawValue:
      ...
      let endPointName = endPointNames[indexPath.row]
      cell.textLabel?.text = endPointName
      cell.accessoryType = endPointName == activeEndPoint ? .checkmark : .none
      return cell
  ...
  func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
    if indexPath.section == EnvironmentSection.endPoint.rawValue {
      interactor.updateCurrentEndPoint(index: indexPath.row)
